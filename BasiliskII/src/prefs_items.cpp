...
std::unordered_map<std::string, prefs_desc> common_prefs_items = {
...
void AddPrefsDefaults(void)
{
	...
	PrefsReplaceInt32("udpport", 6066);
	PrefsReplaceInt32("bootdriver", 0);
	PrefsReplaceInt32("bootdrive", 0);
	PrefsReplaceInt32("ramsize", 8 * 1024 * 1024);
	PrefsReplaceInt32("frameskip", 6);
	PrefsReplaceInt32("modelid", 26);	// Quadra 950
	PrefsReplaceInt32("cpu", 4);		// 68040
	PrefsReplaceInt32("displaycolordepth", 0);
	PrefsReplaceBool("fpu", false);
	PrefsReplaceBool("nocdrom", false);
	PrefsReplaceBool("nosound", false);
	PrefsReplaceBool("noclipconversion", false);
	PrefsReplaceBool("nogui", false);
	
#if USE_JIT
	// JIT compiler specific options
	PrefsReplaceBool("jit", true);
	PrefsReplaceBool("jitfpu", true);
	PrefsReplaceBool("jitdebug", false);
	PrefsReplaceInt32("jitcachesize", 8192);
	PrefsReplaceBool("jitlazyflush", true);
	PrefsReplaceBool("jitinline", true);
#else
	PrefsReplaceBool("jit", false);
#endif

    PrefsReplaceInt32("keyboardtype", 5);
}
