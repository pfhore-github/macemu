...
void reset_all() {
    ...
    r->a[0] = ROMBaseMac + UniversalInfo +
              ReadMacInt32(ROMBaseMac + UniversalInfo); // AddrMap
...
/*
 *  Execute EMUL_OP opcode (called by 68k emulator or Illegal Instruction trap
 * handler)
 */
...
void EmulOp(uint16 opcode, M68kRegisters *r) {
    ...
    case M68K_EMUL_BREAK: { // Breakpoint
        ...
        printf("d0 %08x d1 %08x d2 %08x d3 %08x\n"
               ...
               r->d[7], r->a[0], r->a[1], r->a[2], r->a[3], r->a[4], r->a[5],
               r->a[6], r->a[7], r->sr);
        VideoQuitFullScreen();
#ifdef ENABLE_MON
    ...
    case M68K_EMUL_OP_CLKNOMEM: { // Clock/PRAM operations
        ...
        if((r->d[1] & 0x78) == 0x38) {
            ...
            if(is_read) {
                r->d[2] = XPRAM[reg];
                bool localtalk =
                    !(XPRAM[0xe0] || XPRAM[0xe1]); // LocalTalk enabled?
                switch(reg) {
                case 0x08:
    ...
    case M68K_EMUL_OP_FIX_MEMSIZE: { // Set correct logical and physical memory
                                     // size
        D(bug("Fix MemSize\n"));
        uint32 diff =
            ReadMacInt32(0x1ef8) -
            ReadMacInt32(
                0x1ef4); // Difference between logical and physical size
    ...
    case M68K_EMUL_OP_INSTALL_DRIVERS: { // Patch to install our own drivers
                                         // during startup
        // Install drivers
    ...
    case M68K_EMUL_OP_SCSI_DISPATCH: {          // SCSIDispatch() replacement
        ...
        case 2:  // SCSISelect
        case 11: // SCSISelAtn
            WriteMacInt16(r->a[7] + 2,
                          SCSISelect(ReadMacInt16(r->a[7]) & 0xff));
            stack = 2;
            break;
        case 3: // SCSICmd
            WriteMacInt16(r->a[7] + 6,
                          SCSICmd(ReadMacInt16(r->a[7]),
                                  Mac2HostAddr(ReadMacInt32(r->a[7] + 2))));
            stack = 6;
            break;
        case 4: // SCSIComplete
            WriteMacInt16(r->a[7] + 12,
                          SCSIComplete(ReadMacInt32(r->a[7]),
        ...
        r->a[0] =
            ret; // "rtd" emulation, a0 = return address, a1 = new stack pointer
        r->a[1] = r->a[7] + stack;
        break;
    }
    ...
    case M68K_EMUL_OP_IRQ: // Level 1 interrupt
        ...
        if(InterruptFlags & INTFLAG_60HZ) {
            ...
            if(HasMacStarted()) {
                ...
                r->d[0] =
                    1; // Flag: 68k interrupt routine executes VBLTasks etc.
            }
        }
    ...
    case M68K_EMUL_OP_EXTFS_COMM: // External file system routines
        WriteMacInt16(r->a[7] + 14, ExtFSComm(ReadMacInt16(r->a[7] + 12),
                                              ReadMacInt32(r->a[7] + 8),
                                              ReadMacInt32(r->a[7] + 4)));
        break;

    case M68K_EMUL_OP_EXTFS_HFS:
        WriteMacInt16(r->a[7] + 20, ExtFSHFS(ReadMacInt32(r->a[7] + 16),
