...
uint32_t CPU::ea_ax(uint32_t ax) {
	...
	if( (nw >> 8) & 1 ) {
		...
		if( !((nw >> 6 ) & 1) ) {
			...
			case 2 : return mmu->read_l(ax+bd+rv) + (int16_t)fetch_w();
			case 3 : return mmu->read_l(ax+bd+rv) + fetch_l();
			case 4 : ILLEGAL(this);
			case 5 : return mmu->read_l(ax+bd)+rv;
			case 6 : return mmu->read_l(ax+bd)+rv + (int16_t)fetch_w();
			...
			case 2 : return mmu->read_l(bd+rv) + (int16_t)fetch_w(); 
