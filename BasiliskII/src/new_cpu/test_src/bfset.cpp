...
BOOST_AUTO_TEST_CASE(result)
{
	...
	BOOST_TEST( xcpu.D[0] == 0x12f45678 );
...
BOOST_AUTO_TEST_CASE(width32)
{
	...
	xcpu.D[0] = 0x12345678;
	test_cpu({ 0167300, 0 });
	BOOST_TEST( xcpu.D[0] == 0xffffffff );
}
...
BOOST_AUTO_TEST_CASE(wrap)
{
	...
	BOOST_TEST( xcpu.D[0] == 0xf234567f );
}
	...
	)
{
	...
	test_cpu({ (uint16_t)(0167300 | dst), (uint16_t)(8 << 6 | 4) });
	BOOST_TEST( xcpu.D[dst] == 0x12f45678 );
}
	...
	)
{
	test_write8(0x1000, 0x12 );	
	xcpu.A[dst] = 0x1000;
	test_cpu({ (uint16_t)(0167320 | dst), (uint16_t)(4 << 6 | 4) });
	BOOST_TEST( test_read8(0x1000) == 0x1f );
}
	...
	)
{
	...
	xcpu.A[dst] = 0x1000;
	test_cpu({ (uint16_t)(0167350 | dst), (uint16_t)(4 << 6 | 4), 0x10 });
	BOOST_TEST( test_read8(0x1010) == 0x1f );
}
	...
	)
{
	...
	xcpu.A[dst] = 0x1000;
	xcpu.D[0] = 0x10;
	test_cpu({ (uint16_t)(0167360 | dst), (uint16_t)(4 << 6 | 4), 0x0830 });
	BOOST_TEST( test_read8(0x1040) == 0x1f );
}

BOOST_AUTO_TEST_CASE(xxxw) {
	test_write8(0x1000, 0x12 );	
	test_cpu({ 0167370, (uint16_t)(4 << 6 | 4), 0x1000 });
	BOOST_TEST( test_read8(0x1000) == 0x1f );
}

BOOST_AUTO_TEST_CASE(xxxl) {
	test_write8(0x1000, 0x12 );	
	test_cpu({ 0167371, (uint16_t)(4 << 6 | 4), 0, 0x1000 });
	BOOST_TEST( test_read8(0x1000) == 0x1f );
}
