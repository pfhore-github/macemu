...
uint8_t EA_READ8(int type, int reg, bool override) {
	switch( EA_OP{type} ) {
        case EA_OP::DR     : return regs.d[ reg];
	    case EA_OP::AR     : ILLEGAL_INST(); 
        case EA_OP::EXT2 : 
			if( reg < 4) {
				break;
...
uint16_t EA_READ16(int type, int reg, bool override) {
	switch( EA_OP{type} ) {
        case EA_OP::DR     : return regs.d[ reg];
	    case EA_OP::AR     : return regs.a[ reg ]; 
        case EA_OP::EXT2 : 
			if( reg < 4) {
				break;
			}
...
void EA_WRITE8(int type, int reg, uint8_t v) {
    switch( EA_OP{type} ) {
        case EA_OP::DR     : regs.d[ reg] = (regs.d[reg] & 0xffffff00) | v; return;
	    case EA_OP::AR     : ILLEGAL_INST(); 
        default :
        if( ! regs.i_ea ) {
...
void EA_WRITE16(int type, int reg, uint16_t v) {
    switch( EA_OP{type} ) {
        case EA_OP::DR     : regs.d[ reg] = (regs.d[reg] & 0xffff0000) | v; return;
	    case EA_OP::AR     : regs.a[ reg] = (int16_t)v;  return;
        default :
        if( ! regs.i_ea ) {
...
void EA_WRITE32(int type, int reg, uint32_t v) {
            switch( EA_OP{type} ) {
        case EA_OP::DR     : regs.d[ reg] = v; return;
	    case EA_OP::AR     : regs.a[ reg] = (int16_t)v; return;
        default :
        if( ! regs.i_ea ) {
