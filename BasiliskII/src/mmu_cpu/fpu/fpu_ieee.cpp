...
PRIVATE inline int FFPU get_fp_value (uae_u32 opcode, uae_u16 extra, fpu_register & src)
{
	...
	case 0:
		switch (size) {
		case 6:
			src = (fpu_register) (uae_s8) regs.d[reg];
			break;
		case 4:
			src = (fpu_register) (uae_s16) regs.d[reg];
			break;
		case 0:
			src = (fpu_register) (uae_s32) regs.d[reg];
			break;
		case 1:
			src = make_single(regs.d[reg]);
			break;
		default:
			return 0;
		}
	...
	case 4:
		...
		break;
	case 5:
		ad = regs.a[reg] + (uae_s32) (uae_s16) FETCH();
		break;
	case 6:
//		ad = get_disp_ea_020 (regs.a[reg], FETCH());
		break;
	case 7:
		switch (reg) {
		case 0:
			ad = (uae_s32) (uae_s16) FETCH();
			break;
		case 1:
			ad = FETCH32();
			break;
...
PRIVATE inline int FFPU put_fp_value (uae_u32 opcode, uae_u16 extra, fpu_register const & value)
{
	...
	case 0:
		switch (size) {
		case 6:
			regs.d[reg] = ((toint(value) & 0xff)
									 | (regs.d[reg] & ~0xff));
			break;
		case 4:
			regs.d[reg] = ((toint(value) & 0xffff)
									 | (regs.d[reg] & ~0xffff));
			break;
		case 0:
			regs.d[reg] = toint(value);
			break;
		case 1:
			regs.d[reg] = extract_single(value);
			break;
		default:
			return 0;
		}
...
{
	fpu_debug(("fdbcc_opp %X, %X at %08lx\n", (uae_u32)opcode, (uae_u32)extra, regs.pc));

	uaecptr pc = (uae_u32) regs.pc;
	uae_s32 disp = (uae_s32) (uae_s16) FETCH();
	...
	if (cc == -1) {
		...
		/*
		regs.d[reg] = ((regs.d[reg] & ~0xffff)
				| ((regs.d[reg] - 1) & 0xffff));
		*/
		regs.d[reg] = ((regs.d[reg] & 0xffff0000)
				| (((regs.d[reg] & 0xffff) - 1) & 0xffff));


		// condition reversed.
		// if ((regs.d[reg] & 0xffff) == 0xffff)
		if ((regs.d[reg] & 0xffff) != 0xffff)
		JUMP (pc + disp);
	}
}
...
{
	fpu_debug(("fbcc_opp %X, %X at %08lx, jumpto=%X\n", (uae_u32)opcode, (uae_u32)extra, regs.pc, extra ));
...
void FFPU fpuop_save(uae_u32 opcode)
{
	...
	if ((opcode & 0x38) == 0x18) {
		regs.a[ opcode & 7] = ad; // Never executed on a 68881
...
//		} else if ((opcode & 0x38) == 1) {
			...
			if (extra & 0x2000) { // dr bit
				if (extra & 0x1000) {
					// according to the manual, the msb bits are always zero.
					regs.a[opcode & 7] = get_fpcr() & 0xFFFF;
					fpu_debug(("FMOVEM FPU fpcr (%X) -> A%d\n", get_fpcr(), opcode & 7));
				}
				if (extra & 0x0800) {
					regs.a[opcode & 7] = get_fpsr();
					fpu_debug(("FMOVEM FPU fpsr (%X) -> A%d\n", get_fpsr(), opcode & 7));
				}
				if (extra & 0x0400) {
					regs.a[opcode & 7] = FPU instruction_address;
					fpu_debug(("FMOVEM FPU instruction_address (%X) -> A%d\n", FPU instruction_address, opcode & 7));
				}
			} else {
				if (extra & 0x1000) {
					set_fpcr( regs.a[opcode & 7] );
					fpu_debug(("FMOVEM A%d (%X) -> FPU fpcr\n", opcode & 7, get_fpcr()));
				}
				if (extra & 0x0800) {
					set_fpsr( regs.a[opcode & 7] );
					fpu_debug(("FMOVEM A%d (%X) -> FPU fpsr\n", opcode & 7, get_fpsr()));
				}
				if (extra & 0x0400) {
					FPU instruction_address = regs.a[opcode & 7];
			...
			if ((extra & 0x2000) == 0) {
				...
				if (extra & 0x0400) {
					FPU instruction_address = FETCH32();
			...
			if ((opcode & 0x38) == 0x18) // post-increment?
				regs.a[opcode & 7] = ad;
			if ((opcode & 0x38) == 0x20) // pre-decrement?
//				regs.a[opcode & 7] = ad - 12;
				regs.a[opcode & 7] = ad - incr;
		}
		dump_registers( "END  ");
		return;
	case 6:
