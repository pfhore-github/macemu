...
BOOST_DATA_TEST_CASE(math_const, dbl_off ^ dbl_const, offset, value) {
    ...
    raw_write16(2, 0x5C00 | 2 << 7 | offset);
    m68k_do_execute();
    BOOST_CHECK_CLOSE(static_cast<double>(fpu.fp[2]), value, 1e-08);
}
...
BOOST_DATA_TEST_CASE(exp10_const, bdata::xrange(14) ^ exps, i, value) {
    raw_write16(0, 0171000);
    raw_write16(2, 0x5C00 | 2 << 7 | (0x32 + i));
    m68k_do_execute();
    MPFR_DECL_INIT(tmp, 64);
    mpfr_log10(tmp, fpu.fp[2].ptr(), MPFR_RNDN);
