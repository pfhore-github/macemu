...
BOOST_DATA_TEST_CASE(n, SIGN, sg) {
    auto v1 = copysign(1.0, sg);
    fpu_test(0x3A, v1, 0.0, 0.0);
    BOOST_TEST(fpu.FPSR.n == (sg < 0));
}

BOOST_DATA_TEST_CASE(z, BIT, value) {
    auto v1 = value ? 0.0 : 1.0;
    fpu_test(0x3A, v1, 1.0, 1.0);
    BOOST_TEST(fpu.FPSR.z == value);
}

BOOST_DATA_TEST_CASE(inf, BIT, value) {
    auto v1 = value ? INFINITY : 1.0;
    fpu_test(0x3A, v1, 1.0, 1.0);
    BOOST_TEST(fpu.FPSR.i == value);
}

BOOST_DATA_TEST_CASE(nan_, BIT, value) {
    auto v1 = value ? NAN : 1.0;
    fpu_test(0x3A, v1, 1.0, 1.0);
    BOOST_TEST(fpu.FPSR.nan == value);
}
