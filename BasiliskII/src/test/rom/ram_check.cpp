...
using namespace ROM;
void prepare(MB_TYPE m, const MACHINE_TABLE_T* mc, const MOTHERBOARD_TABLE_T* mb) {
	fixture f(m);
	delete [] RAMBaseHost;
...
BOOST_DATA_TEST_CASE( success, bdata::xrange(1, 8), n ) {
	prepare(MB_TYPE::GLU, &old_machines[0], &motherboards[0]);
...
BOOST_AUTO_TEST_CASE( fail_A ) {
	prepare(MB_TYPE::GLU, &old_machines[0], &motherboards[0]);
	uint32_t sz = 0x100000 << 6;
	GET_MOCK(check_simm_size)
		.nth_is(1, { sz, 0u }, 0x04000000, 0x08000000, CHECK_DATA, 0x100000 );
	GET_MOCK(check_simm)
...
BOOST_AUTO_TEST_CASE( fail_B ) {
	prepare(MB_TYPE::GLU, &old_machines[0], &motherboards[0]);
	constexpr uint32_t sz = 0x100000 << 6;
	GET_MOCK(check_simm_size)
		.nth_is(1, { sz, 0u }, 0, 0x04000000, CHECK_DATA, 0x100000 );
	GET_MOCK(check_simm)
		.nth_is(1, std::bitset<4>("0000"), 0, CHECK_DATA )
...
BOOST_DATA_TEST_CASE( success, bdata::xrange(1, 8), n ) {
	prepare(MB_TYPE::MDU, &old_machines[3], &motherboards[1]);
	uint32_t sz = 0x100000 << (n-1);
	GET_MOCK(check_simm_size)
		.nth_is(1, { sz, 0u }, 0, 0x04000000, CHECK_DATA, 0x100000 )
		.nth_is(2, { sz, 0u }, 0x04000000, 0x08000000, CHECK_DATA, 0x100000 );
	GET_MOCK(check_simm)
		...
		.nth_is(2, std::bitset<4>("0000"), 0x04000000, CHECK_DATA );
	TEST_ROM( A0000 );
	uint32_t mx = 0x04000000 + sz;
	BOOST_CHECK_EQUAL( read_l( mx-28), 0 ); // BASE[0]
	BOOST_CHECK_EQUAL( read_l( mx-24), sz ); // SIZE[0]
	BOOST_CHECK_EQUAL( read_l( mx-20), 0x04000000 ); // BASE[1]
	BOOST_CHECK_EQUAL( read_l( mx-16), sz ); // SIZE[1]
	BOOST_CHECK_EQUAL( read_l( mx-12), 0xffffffff ); // sentinel
	BOOST_CHECK_EQUAL( read_l( mx-8), 0 ); // padding
	BOOST_CHECK_EQUAL( read_l( mx-4), 0 ); // padding
	BOOST_CHECK( cpu.Z );
}
...
BOOST_DATA_TEST_CASE( success_1, bdata::xrange(1, 8), n ) {
	prepare(MB_TYPE::OSS, &old_machines[5], &motherboards[2]);
	uint32_t sz = 0x100000 << (n-1);
	GET_MOCK(check_simm_size)
		.nth_is(1, { sz, 0u }, 0, 0x04000000, CHECK_DATA, 0x100000 )
		.nth_is(2, { sz, 0u }, 0x04000000, 0x08000000, CHECK_DATA, 0x100000 );
	...
	BOOST_CHECK_EQUAL( oss->UT_IIFX_exp0_GET( 0xff ), 0xF300 | OSS_RAMSIZE[n] );
	BOOST_CHECK_EQUAL( read_l( 2*sz-28), 0 ); // BASE[0]
	BOOST_CHECK_EQUAL( read_l( 2*sz-24), sz ); // SIZE[0]
	BOOST_CHECK_EQUAL( read_l( 2*sz-20), sz ); // BASE[1]
	BOOST_CHECK_EQUAL( read_l( 2*sz-16), sz ); // SIZE[1]
...
BOOST_DATA_TEST_CASE( success_2, bdata::xrange(1, 8), n ) {
	prepare(MB_TYPE::OSS, &old_machines[5], &motherboards[2]);
	uint32_t sz = 0x100000 << (n-1);
	...
	auto oss = std::dynamic_pointer_cast<IIFX_exp0>(machine->exp0 );
	TEST_ROM( A0000 );
	BOOST_CHECK_EQUAL( oss->UT_IIFX_exp0_GET( 0xff ), 0xF3EF & ( 0xff00 | OSS_RAMSIZE[n]) );
	BOOST_CHECK( cpu.Z );
}
...
BOOST_DATA_TEST_CASE( success_1, bdata::xrange(1, 5), n ) {
	...
	GET_MOCK(check_simm_size)
		...
		.nth_is(2, { sz, 0u }, 0, 0x00800000, CHECK_DATA, 0x100000 )
		.nth_is(3, { sz, 0u }, 0x00800000, 0x00A00000, CHECK_DATA, 0x100000 );
	GET_MOCK(check_simm)
		.nth_is(1, std::bitset<4>("0000"), 0, CHECK_DATA )
		.nth_is(2, std::bitset<4>("0000"), 0x00800000, CHECK_DATA );
...
BOOST_AUTO_TEST_CASE( success_2 ) {
	...
	RAMSize = sz;
	GET_MOCK(check_simm_size)
		.nth_is(1, { 0x100000, 0u }, 0, 0x00800000, CHECK_DATA, 0x100000 );
	GET_MOCK(check_simm)
		.nth_is(1, std::bitset<4>("0000"), 0, CHECK_DATA )
		.nth_is(2, std::bitset<4>("1111"), 0x00800000, CHECK_DATA );
	auto rbv = std::make_shared<IO_TEST<RBV>>();
...
BOOST_DATA_TEST_CASE( success_3, bdata::xrange(1, 5), n ) {
	...
	GET_MOCK(check_simm_size)
		...
		.nth_is(2, { sz, 0u }, 0, 0x00400000, CHECK_DATA, 0x100000 )
		.nth_is(3, { sz, 0u }, 0x00400000, 0x00800000, CHECK_DATA, 0x100000 );
	GET_MOCK(check_simm)
		.nth_is(1, std::bitset<4>("0000"), 0, CHECK_DATA )
		.nth_is(2, std::bitset<4>("0000"), 0x00400000, CHECK_DATA );
	auto rbv = std::make_shared<IO_TEST<RBV>>();
...
BOOST_DATA_TEST_CASE( success, bdata::xrange(1, 4), n ) {
	...
	machine->rbv = rbv;
	rbv->set_read_data( 16, { 0x0f, 0xff } );
	uint32_t sz = sz1+sz2;
	TEST_ROM( A0000 );
	if( n == 3 ) {
		rbv->verify( 16, { 0xff, 0xff } );
	} else {
		rbv->verify( 16, { 0xff, 0xef } );
	}
	for(int i = 0; i < 4; ++i ) {
...
BOOST_DATA_TEST_CASE( success_1, bdata::xrange(1, 4), n ) {
	...
	auto rbv = std::make_shared<IO_TEST<RBV>>();
	machine->rbv = rbv;
	rbv->set_read_data( 1, { 0x80, 0x80 } );
	TEST_ROM( A0000 );
	if( n == 4 ) {
		rbv->verify( 1, { 0xff, 0xff } );
	} else {
		rbv->verify( 1, { 0xff, uint8_t(0xc0 | n << 3 | n) } );
	}
#if 0
	BOOST_CHECK_EQUAL( read_l( 4*sz-76), 0 ); // BASE[0]
	BOOST_CHECK_EQUAL( read_l( 4*sz-72), sz ); // SIZE[0]
